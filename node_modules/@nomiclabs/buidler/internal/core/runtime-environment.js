...
class Environment {
     ...
     */
    constructor(config, buidlerArguments, tasks, extenders = [], experimentalBuidlerEVMMessageTraceHooks = []) {
        this.config = config;
        ...
        const provider = lazy_1.lazyObject(() => {
            ...
            return construction_1.createProvider(networkName, networkConfig, config.solc.version, config.paths, experimentalBuidlerEVMMessageTraceHooks.map((hook) => (trace, isCallMessageTrace) => hook(this, trace, isCallMessageTrace)));
        });
        this.network = {
            name: networkName,
    ...
    _resolveArgument(paramDefinition, argumentValue) {
        ...
        if (argumentValue === undefined) {
            ...
            throw new errors_1.BuidlerError(errors_list_1.ERRORS.ARGUMENTS.MISSING_TASK_ARGUMENT, {
                param: name,
            });
        }
