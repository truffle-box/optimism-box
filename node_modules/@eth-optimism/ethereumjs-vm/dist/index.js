...
var ovm_state_manager_1 = require("./ovm/ovm-state-manager");
...
var VM = /** @class */ (function (_super) {
    ...
    function VM(opts) {
        ...
        // OVM option setup.
        var ovmOpts = opts.ovmOpts || {};
        _this.emGasLimit = ovmOpts.emGasLimit || 100000000;
        _this.dump = ovmOpts.dump;
        _this.initialized = ovmOpts.initialized || false;
        if (ovmOpts.contracts) {
            _this.contracts = ovmOpts.contracts;
        }
        else {
            if (!_this.dump) {
            ...
            _this.contracts = {};
            for (var _i = 0, _a = Object.entries(_this.dump.accounts); _i < _a.length; _i++) {
                var _b = _a[_i], name = _b[0], account = _b[1];
                _this.contracts[name] = {
                    name: name,
                    address: buffer_utils_1.fromHexString(account.address),
                    addressHex: account.address,
                    code: account.code,
                    iface: new abi_1.Interface(account.abi),
                };
            }
        }
        // Always need an instance of this.
        _this.ovmStateManager = new ovm_state_manager_1.OvmStateManager({
            vm: _this,
        });
        // We cache this promisified function as it's called from the main execution loop, and
        // promisifying each time has a huge performance impact.
        var emit = promisify(_this.emit.bind(_this));
        _this._emit = function (topic, data) { return __awaiter(_this, void 0, void 0, function () {
            var addresses, _i, addresses_1, address, addr;
            return __generator(this, function (_a) {
                if (data) {
                    addresses = [data.address, data.to];
                    for (_i = 0, addresses_1 = addresses; _i < addresses_1.length; _i++) {
                        address = addresses_1[_i];
                        if (address) {
                            addr = address.toString('hex');
    ...
    VM.prototype.init = function () {
        return __awaiter(this, void 0, void 0, function () {
            var _i, _a, account, _b, _c, _d, key, val;
            return __generator(this, function (_e) {
                switch (_e.label) {
                    case 0:
                        ...
                        _i = 0, _a = Object.values(this.dump.accounts);
                        _e.label = 1;
                    case 1:
                        if (!(_i < _a.length)) return [3 /*break*/, 8];
                        account = _a[_i];
                        return [4 /*yield*/, this.pStateManager.putAccount(buffer_utils_1.fromHexString(account.address), new ethereumjs_account_1.default())];
                    case 2:
                        _e.sent();
                        return [4 /*yield*/, this.pStateManager.putContractCode(buffer_utils_1.fromHexString(account.address), buffer_utils_1.fromHexString(account.code))];
                    case 3:
                        _e.sent();
                        _b = 0, _c = Object.entries(account.storage);
                        _e.label = 4;
                    case 4:
                        if (!(_b < _c.length)) return [3 /*break*/, 7];
                        _d = _c[_b], key = _d[0], val = _d[1];
                        return [4 /*yield*/, this.pStateManager.putContractStorage(buffer_utils_1.fromHexString(account.address), buffer_utils_1.fromHexString(key), buffer_utils_1.fromHexString(val))];
                    case 5:
                        _e.sent();
                        _e.label = 6;
                    case 6:
    ...
    VM.prototype.getContract = function (address) {
        return Object.values(this.contracts).find(function (contract) {
            return contract.address.equals(address);
        });
    };
    VM.prototype.getContractByName = function (name) {
        return Object.values(this.contracts).find(function (contract) {
