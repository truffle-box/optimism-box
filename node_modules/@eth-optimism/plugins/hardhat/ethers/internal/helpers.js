...
exports.collectLibrariesAndLink = exports.getContractFactory = exports.getContractAt = exports.getSigners = void 0;
const plugins_1 = require("hardhat/plugins");
const pluginName = '@eth-optimism/plugins/hardhat';
exports.getSigners = async (hre) => {
    const { SignerWithAddress: SignerWithAddressImpl } = await Promise.resolve().then(() => __importStar(require('./signer-with-address')));
    const accounts = await hre.l2ethers.provider.listAccounts();
    const signers = accounts.map((account) => hre.l2ethers.provider.getSigner(account));
    const signersWithAddress = await Promise.all(signers.map(SignerWithAddressImpl.create));
    return signersWithAddress;
};
const addGasToAbiMethodsIfNecessary = (networkConfig, abi) => {
...
const getContractFactoryByAbiAndBytecode = async (hre, abi, bytecode, signer) => {
    const { ContractFactory } = require('ethers');
    if (signer === undefined) {
        const signers = await hre.l2ethers.getSigners();
        signer = signers[0];
    }
...
exports.getContractAt = async (hre, nameOrAbi, address, signer) => {
...
const getContractFactoryByName = async (hre, contractName, signerOrOptions) => {
    var _a;
...
If you want to call a contract using ${contractName} as its interface use the "getContractAt" function instead.`);
    ...
    const linkedBytecode = await exports.collectLibrariesAndLink(artifact, libraries);
    return getContractFactoryByAbiAndBytecode(hre, artifact.abi, linkedBytecode, signer);
};
exports.getContractFactory = async (hre, nameOrAbi, bytecodeOrFactoryOptions, signer) => {
    if (typeof nameOrAbi === 'string') {
...
exports.collectLibrariesAndLink = async (artifact, libraries) => {
